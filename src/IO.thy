...
fun read_array :: "'a array \<Rightarrow> nat \<Rightarrow> 'a::repr io" where
  "read_array (Array arr _) i = IO (State (get_world (arr + i)))"

fun write_array :: "'a array \<Rightarrow> nat \<Rightarrow> 'a::repr \<Rightarrow> unit io" where
  "write_array (Array arr _) i val = IO (State (put_world (arr + i) val))"
...
lemma monad_left_id: "return a \<bind> f = f a"
  apply (simp add: return_def bind_def comp_def)
  done

lemma monad_right_id: "m \<bind> return = m"
  apply (simp add: return_def bind_def comp_def)
  done
